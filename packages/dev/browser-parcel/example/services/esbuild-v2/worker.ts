...
import { NPMInstaller } from '../esbuild/dependencies-installer'
import { FileSystemManager } from '../file-system-manager'
import { setFilesBulk } from '../esbuild/helpers/fs'
...
const setup = async ([_event, version, wasm]: ['setup', string, ArrayBuffer]): Promise<API> => {
...
const handler = function<R extends IPCRequest>(this: API, e: MessageEvent<R>) {
  ...
  const respond: (response: IPCResponse<IPCRequest>) => void = postMessage
  let start: number
  let color = true
  ...
  try {
    ...
    // Build API
      ...
      start = perf.now()
      console.log('build', request.options_)
      this.build(request.options_).then(
        ({ warnings, outputFiles, metafile, mangleCache }) =>
          finish(warnings, (stderr: string) =>
            respond({
              outputFiles_: outputFiles.map(({ path, contents }: any) => ({ path, contents })),
              metafile_: metafile,
              mangleCache_: mangleCache,
              stderr_: stderr,
              duration_: perf.now() - start,
    ...
    // Npm Install API
      ...
      console.log('npm_install', request.input_)
