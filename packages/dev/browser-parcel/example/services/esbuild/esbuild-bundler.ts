...
import type { BundleOptions as BaseBundleOptions, Worker as RawEsbuildWorker } from './esbuild-worker';
...
export async function initWorker() {
  ...
  const worker = comlink.wrap<RawEsbuildWorker>(
    new Worker(
      new URL('./esbuild-worker.ts', import.meta.url),
      { type: 'module' },
    ),
  );
  ...
  return {
    ...
    esbuild__resolvePackage: worker.esbuild__resolvePackage,
    esbuild__bundle: async (options: BundleOptions) => {
      const result = await worker.esbuild__bundle(options);
...
export type EsbuildWorker = PromiseReturnType<ReturnType<typeof initWorker>>;
