...
import type { Plugin as RollupPlugin } from '@rollup/browser'
import { transform, type Loader, type TransformOptions, type BuildOptions as EsbuildOptions } from 'esbuild-wasm'
...
export default function rollupPluginEsbuild({
  fs,
  include,
  exclude,
  sourceMap = true,
  ...
  return {
    ...
    async resolveId(id, importer) {
      ...
      if (importer && id[0] === '.') {
        ...
        if (file) return file
        if (!file && fs.exists(resolved) && fs.isDirectory(resolved)) {
    ...
    async transform(code, id) {
      ...
      const tsconfigRaw =
        tsconfig === false
          ? undefined
          : getTsconfig(fs, id, tsconfig || 'tsconfig.json')
