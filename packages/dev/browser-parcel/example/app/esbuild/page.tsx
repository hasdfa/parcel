...
import { initWorker, EsbuildWorker } from '@/services/esbuild/esbuild-bundler';

const PROJECT_ID = 'esbuild';

const MultiFileCodeEditor = ({ worker: { fs, esbuild__bundle, npm__install, esbuild__resolvePackage } }: { worker: EsbuildWorker }) => {
  // Default files setup
  ...
  const compileCode = React.useCallback(async () => {
    ...
    try {
      ...
      await npm__install({
        ...
        reported: (type: 'error' | 'info', message: string) => {
      ...
      const result = await esbuild__bundle({
        projectId: PROJECT_ID,
        entries: Object.entries(fs.files).filter(([path, file]) => file.jsEntry).map(([path]) => path),
      });
  ...
  }, [fs, esbuild__bundle, npm__install, esbuild__resolvePackage]);
...
export default function Wrapper() {
  ...
  React.useEffect(() => {
    initWorker().then((worker) => {
      worker.fs.setFiles(
        Object.fromEntries(Object.entries(defaultFiles).map(([path, file]) => [`/app/${path}`, file])),
      );
