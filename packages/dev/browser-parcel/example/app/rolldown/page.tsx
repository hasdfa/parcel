...
import { initWorker, RollupWorker } from '@/services/rolldown';

const PROJECT_ID = 'rollup';

const MultiFileCodeEditor = ({ worker: { fs, rolldown__bundle, npm__install } }: { worker: RollupWorker }) => {
  // Default files setup
  ...
  const compileCode = React.useCallback(async () => {
    ...
    try {
      ...
      const result = await rolldown__bundle({
        projectId: PROJECT_ID,
        input: Object.entries(fs.files).find(([path, file]) => file.jsEntry)?.[0] || '/app/index.js',
      });
  ...
  }, [fs, rolldown__bundle, npm__install]);
...
export default function Wrapper() {
  const [result, setResult] = useState<RollupWorker | null>(null);
