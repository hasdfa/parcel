...
// https://github.com/parcel-bundler/parcel/blob/d6a4df3700bc2364eca18c25c42aa5f220c4b8db/packages/core/core/src/index.js#L4-L11
...
// Custom plugins to make it work
// import reporterServer from '../parcel-plugins/dev-server-sv';
...
export class BrowserPackageManager implements PackageManager {
  ...
  async require(
    ...
    if (resolved in BUILTINS) {
      return BUILTINS[resolved as keyof typeof BUILTINS];
    }
  ...
  async resolve(
    ...
    if (!resolved) {
      ...
      let getPkg: () => any;
      switch ((res.resolution as any).type) {
        case 'Path':
          getPkg = () => {
            let pkgPath = this.fs.findAncestorFile(
              ['package.json'],
              (res.resolution as any).value!,
              this.projectRoot,
            );
            resolved = pkgPath
        ...
        case 'Builtin':
          resolved = {
            resolved: (res.resolution as any).value,
