...
export class NPMInstaller {
  ...
  private static async getPackageJson(fs: FileSystem, options: NPMSpawnOptions) {
    ...
    const result = content ? JSON.parse(new TextDecoder().decode(content)) : null;
    return result;
  }
  ...
  public static async install(
    ...
    const scripts: Record<string, string> = {
      ...(await fs.readFile(scriptsPath, 'utf8').then(JSON.parse).catch(() => ({})) || {}),
    };
      ...
      Object.entries(packages).map(([packageName, packageVersion]) => requestsQueue.add(async () => {
        ...
        if (await fs.exists(cachePath)) {
          ...
          for (const [baseFilePath, fileContent] of Object.entries(files)) {
            ...
            await fs.writeFile(filePath, string, undefined);
          }
        }

        // TODO: gather "scripts" from all deps package.json
        const pkgJsonText = await fs.readFile(path.join(cachePath, 'package.json'), 'utf8').catch(() => null);
    ...
    await fs.writeFile(scriptsPath, JSON.stringify(scripts, null, 2), undefined);
...
const IDB_DB_DEPENDENCIES = 'PARCEL-dependencies-cache';
