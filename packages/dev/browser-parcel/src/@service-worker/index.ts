...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === EventsToSW.UPLOAD_FILES) {
    const { projectId, files } = event.data.payload;

    // Store files in memory
    fileStore.set(projectId, files);
    console.log('files uploaded', fileStore);

    // Send confirmation back to main thread
    event.source?.postMessage({
      type: EventsFromSW.UPLOAD_FILES_FINISHED,
      projectId
    });
  }
});
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/__build/')) {
    ...
    console.log('get project files', { projectId, filePath, projectFiles: projectFiles ? Object.keys(projectFiles) : null });
